...
export default function Hero() {
  ...
  const [headBounds, setHeadBounds] = useState({
    left: 0,
    top: 0,
    width: 1,
    height: 1,
  ...
  useEffect(() => {
    const updateBounds = () => {
      if (headContainerRef.current)
        setHeadBounds(headContainerRef.current.getBoundingClientRect());
    };
    updateBounds();
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-8 left-1/2 -translate-x-1/2 z-20 flex flex-col items-center">
        <Link
          to="/"
          className="block focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--hero-accent)] focus-visible:ring-offset-2 rounded-lg"
        >
          {logoError ? (
      ...
      >
        <div className="hero-left max-w-xl text-left w-full">
          <p className="hero-label uppercase mb-3">Welcome to</p>
          <h1 className="hero-heading mb-5">
            Comfort and luxury of uniforms,{" "}
            <span className="text-[#2563eb]">simple</span>.
          </h1>
          <p className="hero-paragraph max-w-md mb-6">
            Premium school uniforms and accessories — thoughtful fabrics,
            precise fits, delivered in 2 days. Easy ordering and hassle-free
            returns.
          </p>
